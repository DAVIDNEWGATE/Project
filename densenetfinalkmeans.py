...
def run_model(data, image_dim, label_count, depth):
  ...
  #Load mask   
  mask_loadpath='100pkmeans64.txt'
  prune_dict = get_dict(mask_loadpath)

  #set weights path
  load_path = 'kmeans/100pkmeans64.ckpt'
  #normal save path
  normal_savepath = 'kmeans2/100pkmeans64_%d.ckpt'
  #best result save path  
  best_savepath = 'kmeans3/100pkmeans64_%d_%dok.ckpt'
  ########################config needed
  
  #trainer = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True)
  trainer = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True)
  #trainer = tf.train.GradientDescentOptimizer(lr)
  
  grads_and_vars = trainer.compute_gradients(cross_entropy)
  grads_and_vars = apply_weightshared_on_grads(grads_and_vars, prune_dict)
